...
# read data
DF = pd.read_csv('ExpCurves_corrected.csv')

# analyze learning rate (True) or learning exponent (False)
lr = True
...
def animate(yy, *fargs):
    ...
    if yy == df['Year'].values[0]:
        ...
        # compute learning exponent from first difference wright's law
        slope = sum(np.diff(y) * np.diff(x)) / sum(np.diff(x)**2)
        ...
        # learning rate based
        if lr:
            ax[0].plot([-200,200],[-200,200], color='k',
        ...
        ## learning rate based
        if lr:
            ax[0].set_xlim(100 * (1 - 2**slope)-20,100 * (1 - 2**slope)+20)
            ax[0].set_ylim(100 * (1 - 2**slope)-20,100 * (1 - 2**slope)+20)
        else:
            ax[0].set_xlim(slope-0.75,slope+0.75)
            ax[0].set_ylim(slope-0.75,slope+0.75)
        ...
        if lr:
            ax[0].set_xlabel('Observed learning rate (%)')
            ax[0].set_ylabel('Future learning rate (%)')
        else:            
    ...
    if yy > df['Year'].values[0]:
        ...
        slopec = sum(np.diff(y_cal) * np.diff(x_cal)) / sum(np.diff(x_cal)**2)
        ...
        slopev = sum(np.diff(y_val) * np.diff(x_val)) / sum(np.diff(x_val)**2)
...
#                  os.path.sep + t +'_lr.gif', writer=writer)
#     else:
#         ani.save('figs' + os.path.sep + 'gifs'+ 
...
df = pd.read_csv('new_solar_series.csv')
df['Tech'] = 'Solar PV'
