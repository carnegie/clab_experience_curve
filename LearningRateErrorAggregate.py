...
slopes['Learning exponent error'] = (slopes['Slope'] 
                                     - slopes['Final estimate'])
...
slopes['Years from first observation'] = (slopes['Intermediate year']
                                          - slopes['Initial year'])
...
statsprod = []
countprod = []
xprod = np.arange(steps[0], 
                  max(slopes['Cumulative production relative to initial']),
                  steps[0])
statstime = []
counttime = []
xtime = np.arange(4, 
                  max(slopes['Years from first observation']),
                  steps[1])
...
ax[1][0].fill_between(10**xprod, 
                      np.array(statsprod)[:,0], 
                      np.array(statsprod)[:,4],
                      color='#57B8FF', alpha=0.1)
ax[1][0].fill_between(10**xprod, 
                      np.array(statsprod)[:,1], 
                      np.array(statsprod)[:,3],
...
ax[1][1].fill_between(xtime, 
                      np.array(statstime)[:,0], 
                      np.array(statstime)[:,4],
                      color='#57B8FF', alpha=0.1)
ax[1][1].fill_between(xtime, 
                      np.array(statstime)[:,1], 
                      np.array(statstime)[:,3], 
                      color='#57B8FF', alpha=0.3)
